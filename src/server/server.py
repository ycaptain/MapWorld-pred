...
class ServerMain:
    ...
    def send_progress(self, total, count, img_path, id, json_path):
        if self.client:
            res = ProgsReq(total, count, img_path, id, json_path)
            self.client.NotifyProgress(res)

    def send_result(self, label_path, json_path, current, total, id):
        if self.client:
            res = ResultReq(label_path, json_path, current, total, id)
    ...
    def pred(self, paths, metas, m_cfg, id):
        ...
        try:
            ...
            if m_typename == "Deeplab" or m_typename == "UNet":
                ...
                self.pred_ths.append(SegPredThread(self, paths, metas, self.tmp_path, id))
                ...
                self.pred_ths.append(CycleGANPredThread(self, paths, metas, self.tmp_path, id))
            else:
